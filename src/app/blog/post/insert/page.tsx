...
export default function Page() {
  ...
  const generateContent = async () => {
    if (!formData?.title) {
      console.error("Title is required to generate content");
      return;
    }
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        setTimeout(() => generateContent(), 1000); // Retry after 1 second
        return;
      }
      ...
      if (data?.choices && data.choices.length > 0) {
        ...
        console.error("No content returned from OpenAI API", data);
      }
    } catch (error) {
